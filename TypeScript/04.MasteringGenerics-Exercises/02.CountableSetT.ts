...
class CountedSet<T> implements CountableSet<T>{
    ...
    private setItems:Map<T,number> = new Map();

    add(item: T): void {

        const currentCount = this.setItems.get(item);
        if(currentCount){
            this.setItems.set(item,currentCount+1);
        }
        else{
            this.setItems.set(item,1);
        }
    }
    ...
    getNumberOfCopies(item: T): number {
        return this.setItems.get(item) ?? 0;
    }
...
//These should return errors in TS 
