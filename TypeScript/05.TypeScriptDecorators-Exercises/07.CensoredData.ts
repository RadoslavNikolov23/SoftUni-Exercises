...
function filterRecentItems(seconds:number){
    return function(target:any, methodName:string,descriptor:PropertyDescriptor){
        const origMethod = descriptor.value;

        descriptor.value = function(){
            const allItems = origMethod.call(this);
...
function censorPropert<T extends { [key:string]:any } > (censoreService:MockCensorService<T>){
    return function (target:any, methodName:string, descriptor:PropertyDescriptor){
        const origMethod = descriptor.value;

        descriptor.value = function () {
            const allItems = origMethod.call(this);
            const censoredItems = censoreService.censorProperties(allItems);
...
function logDecor(target:any, methodName:string, descriptor:PropertyDescriptor){
    const origMethod = descriptor.value;
...
class UsersService {
    ...
    @filterRecentItems(5)
    @censorPropert(userCensorService)
    getUsers() { return this._users; }

    @logDecor
    @filterRecentItems(10)
    @censorPropert(employeeCensorService)
